...
function mentorCopy(stage: MentorStage, codename: string, languageName: string, languageRoute: string, assistantName: string) {
  if (stage === 'branches-unlocked') {
    return {
      kicker: 'ROOT CLEAR · BRANCHES ONLINE',
      title: `${codename}，基础根系打通了。${assistantName} 带你选后续路线。`,
      body: `${languageName} 基础已经够你进入领域分支。接下来别刷散题，去选一个方向做成作品。`,
  ...
  return {
    ...
    title: `${codename}，欢迎接入。${assistantName} 先带你走一圈。`,
    body: `这里不是单纯看视频，也不是把答案喂进编辑器。你会先听小助理讲清楚概念，再自己写代码，用真实运行和测试确认你真的会了。`,
...
function tutorialSteps(stage: MentorStage, languageName: string, assistantName: string): TutorialStep[] {
  if (stage === 'branches-unlocked') {
    return [
      {
        icon: Compass,
        title: '你现在该选方向了',
        body: `基础关不是终点，只是让你看得懂 ${languageName} 程序怎么入口、怎么传数据、怎么把结果交出去。接下来要把这些能力接到真实领域。`,
        bullets: ['想做可视化就进数据/CV', '想做作品展示就进 Pygame/Web', '想做智能应用就进 AI 分支'],
      },
      {
        icon: Sparkles,
        title: '每条分支都要有作品感',
        body: '后面的课程不应该只是“又学一个语法”。目标是做出能运行、能截图、能解释给别人听的小项目。',
  ...
  return [
    {
      icon: Compass,
      title: '先认路：语言、分支、课程',
      body: `仪表盘中间是课程入口。你先选 ${languageName}，再选一个领域分支，最后进入具体课程。这样不会被一堆关卡糊脸。`,
      bullets: ['基础分支负责入口、变量、条件、循环、函数', '领域分支负责把基础能力接到游戏、数据、Web、AI、CV 等方向', '搜索栏可以直接找课程，不必死翻地图'],
    },
    {
      icon: BookOpen,
      title: '每一关先听我讲，再动手',
      body: `${assistantName} 会先用人话讲清楚“这关有什么用、脑子里怎么建模、最容易错在哪里”。读完以后编辑器才上场，而且是空的。`,
      bullets: ['不是复制答案，而是先理解最小结构', '教学页会给术语、真实用途、常见坑和自测问题', '你写出的第一版越短，越容易找到问题'],
    },
    {
      icon: Code2,
      title: '写代码时，我会看上下文',
      body: '你不用每次点运行我才知道发生了什么。停顿、语法异常、运行错误、测试失败都会进入小助理上下文。',
      bullets: ['空编辑器会提醒你先写最小代码', '报错时优先读第一行错误', '测试失败时先对齐目标输出或核心结构'],
    },
    {
      icon: TerminalSquare,
      title: '运行不是仪式，是验证',
      body: `点运行后，${languageName} 代码会交给当前运行模式处理。你要看的是：终端输出、测试检查、以及失败提示里最关键的一处。`,
      bullets: ['先让代码跑起来，再追求漂亮', '一次只修一个问题', '通关后回想：是哪一行真正让目标成立'],
    },
    {
      icon: Settings2,
      title: '把工作台调成你的习惯',
      body: '右上角命令中心可以改代号、小助理人格、字体、面板宽度和活人感。它是学习工具，不是固定死的考试界面。',
...
export function DashboardMentor({ codename, languageName, languageRoute, stage, nextLevelId, settings }: DashboardMentorProps) {
  ...
  const copy = mentorCopy(stage, codename, languageName, languageRoute, persona.name)
  const steps = tutorialSteps(stage, languageName, persona.name)
          ...
          <div className="cn-scrollbar max-h-[calc(100dvh-104px)] space-y-4 overflow-y-auto p-4 sm:p-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-cyan-300/42">
                      小助理教程 {stepIndex + 1}/{steps.length}
                    </p>
                    <h3 className="mt-1 text-sm font-semibold text-white/86">{step.title}</h3>
                  </div>
                </div>
            ...
            {stage === 'new-user' && (
              <div className="rounded-lg border border-cyan-300/14 bg-cyan-300/[0.045] px-3 py-3 text-xs leading-relaxed text-cyan-50/62">
                ...
                <p className="mt-1">
                  进入 Lv.{nextLevelId}，先看教学页，再自己写第一段 {languageName} 代码。过关不是为了亮一个勾，而是证明你能把“概念 → 代码 → 输出”接起来。
                </p>
              </div>
            )}
            ...
            <div className="flex gap-2">
              ...
              {!isLastStep ? (
                ...
                >
                  ...
                  {stage === 'new-user' ? `接入 Lv.${nextLevelId}` : copy.primaryLabel}
                </Link>
              )}
            </div>
