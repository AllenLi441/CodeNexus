...
export function CustomRoutePanel({ route, levels, languageName, guestMode, onChangeLevel, onProfileChange }: Props) {
  ...
  async function generate() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(typeof data.error === 'string' ? data.error : tr('生成失败，稍后再试。'))
        return
      }
      ...
      if (incoming.length < 2) {
        setError(tr('这次没生成出清晰的路线，换个说法再试。'))
        return
      }
  ...
  const header = (
    <div className="mb-2 flex items-center gap-2">
      ...
      <p className="text-xs font-semibold text-white/72">{route ? tr('我的定制路线') : tr('你想做个什么？')}</p>
        ...
        <div className="rounded-lg border border-cyan-300/14 bg-cyan-300/[0.035] p-3">
          ...
          <div className="mb-2 flex items-center justify-between gap-2">
            <p className="min-w-0 flex-1 truncate text-[11px] text-white/45">
              {tr('目标')}：<span className="text-cyan-100/75">{route.goal}</span>
            </p>
                ...
                >
                  ...
                  <div className="mt-1.5 pl-6">
                    ...
                    {linked ? (
                      ...
                      >
                        {tr('去')} Lv.{linked.id} {tr('练手')} <ArrowRight className="h-3 w-3" />
                      </button>
                    ) : null}
                  </div>
                </div>
          ...
          {allDone ? (
            <p className="mt-2 text-[11px] leading-relaxed text-emerald-200/70">
              {tr('整条路线都打勾了 —— 你已经把')}「{route.goal}」{tr('拆开走了一遍。可以重新定制下一个目标。')}
            </p>
          ) : null}
    ...
    return (
      <div className={SECTION_WRAP}>
        <div className="rounded-lg border border-cyan-300/14 bg-cyan-300/[0.035] p-3">
          ...
          <div className="mt-2 grid gap-1.5 opacity-70">
            {GUEST_EXAMPLE.map((step, index) => (
              <div key={step.title} className={`${CARD} border-dashed`}>
                ...
                <p className="mt-0.5 pl-5 text-[11px] leading-relaxed text-white/40">{tr(step.why)} · {tr(step.task)}</p>
              </div>
            ))}
          </div>
          <a
          ...
          >
            {tr('登录后生成我的专属路线')} <ArrowRight className="h-3 w-3" />
          </a>
        </div>
      </div>
    )
    ...
    <div className={SECTION_WRAP}>
      <div className="rounded-lg border border-cyan-300/14 bg-cyan-300/[0.035] p-3">
        ...
        <p className="mb-2 text-[11px] leading-relaxed text-white/45">
          {tr('说出你真正想做的东西，我帮你拆成 4–6 步专属路线，每步标好概念、动手任务，还能跳去练对应关卡。')}
        </p>
        <input
          value={goal}
          ...
          disabled={loading}
          placeholder={tr('记账小工具、猜数字游戏、每日一句…')}
