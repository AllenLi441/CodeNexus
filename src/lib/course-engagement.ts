...
// A `tr` is the same `(zh) => string` lookup the UI uses (useTr() in client
// components, or `(s) => translate(s, lang)` on the server). It does an EXACT
// match against EN_MAP, so display text MUST be wrapped as a whole template
// (with {placeholder} tokens) — never assembled from already-interpolated
// Chinese, or the lookup can't match. Callers that don't want translation can
// pass the identity below to get the original Chinese output unchanged.
type Tr = (zh: string) => string
const identityTr: Tr = (zh) => zh
...
// Returns the intent as a finished DISPLAY fragment (already translated). The
// branch needles match on the ORIGINAL Chinese title/objective and must never be
// translated. The static branches resolve to whole EN_MAP keys; the fallback is
// a {objective} template so it can translate without an exact-match miss.
function levelIntent(level: Level, tr: Tr = identityTr) {
  const text = `${level.title} ${level.objective}`.toLowerCase()
  if (level.id === 20) return tr('把类、函数、集合、异常处理和装饰器融合成一个可运行的小项目')
  if (/hello|输出|print|console|printf/.test(text)) return tr('让程序对外发出第一条可验证信号')
  if (/变量|类型|string|int|double|dim|const|let|var/.test(text)) return tr('把数据命名、分类，并保持后续逻辑读得懂')
  if (/if|else|条件|判断|分支/.test(text)) return tr('让程序在不同情况里做不同选择')
  if (/循环|for|while|array|list|数组|列表|foreach|vector/.test(text)) return tr('让重复工作交给结构化流程处理')
  if (/函数|方法|function|method|add/.test(text)) return tr('把一段逻辑拆成可以复用和测试的单元')
  if (/class|对象|属性|property|user|counter/.test(text)) return tr('把状态和行为收进一个清楚的模型')
  if (/指针|内存|raii|unique_ptr/.test(text)) return tr('直接面对资源和生命周期，别让隐患藏在底层')
  if (/async|await|异步|fetch/.test(text)) return tr('处理等待中的任务，同时保持流程可读')
  return tr('完成目标：{objective}').replace('{objective}', tr(level.objective))
}
...
function teachingWalkthrough(languageName: string, level: Level, concept: string, tr: Tr) {
  ...
  if (level.id === 20) {
    return [
      {
        title: tr('先定游戏世界的骨架'),
        body: tr('最终关不是拼语法清单，而是把角色、物品、动作和输出日志组织成一个能运行的小系统。先把对象关系想清楚，代码才不会散。'),
      },
      {
        title: tr('用函数和方法拆动作'),
        body: tr('像 `{coreLine}` 这样的导入只是工具入口。真正要交付的是：角色能拾取物品、使用物品、处理错误，并输出完整过程。').replace('{coreLine}', coreLine),
      },
      {
        title: tr('异常处理负责兜底'),
        body: tr('项目题最怕只在正常路径能跑。不存在的物品、错误输入、空背包这些情况都要有可解释的反馈，而不是直接崩掉。'),
      },
    ]
  }
  ...
  if (/hello|输出|print|console|printf|write(line)?/.test(text)) {
    return [
      {
        title: tr('入口先说人话'),
        body: tr('{languageName} 程序先执行入口附近的语句。你这一关不是“会背输出函数”，而是能让程序向终端交付一条可验证的信息。').replace('{languageName}', languageName),
      },
      {
        title: tr('引号里是内容，引号外是指令'),
        body: tr('像 `{coreLine}` 这种结构，函数名负责动作，括号负责传参，引号里的文本才是最终被输出的内容。').replace('{coreLine}', coreLine),
      },
      {
        title: tr('测试只认结果'),
        body: tr('肉眼看到类似文本不够，大小写、标点、空格都可能让自动检查失败。训练目标是写出稳定可复现的结果。'),
      },
    ]
  }
  ...
  if (/变量|类型|string|int|double|dim|const|let|var|f-string/.test(text)) {
    return [
      {
        title: tr('先给数据起名字'),
        body: tr('变量把一段数据从“散落的值”变成“可被复用的含义”。名字越具体，后面越少靠猜。'),
      },
      {
        title: tr('类型决定能做什么'),
        body: tr('{languageName} 里的值不是都一样。数字能计算，字符串适合展示，布尔值负责判断。别把它们混着用。').replace('{languageName}', languageName),
      },
      {
        title: tr('输出是一次组装'),
        body: tr('把变量放进文本时，先确认每个变量的值，再确认格式化方式。最后输出的是用户能读懂的句子，不是代码作者的脑补。'),
      },
    ]
  }
  ...
  if (/if|else|条件|判断|分支|switch/.test(text)) {
    return [
      {
        title: tr('条件是岔路口'),
        body: tr('程序不会“理解情况”，它只会判断表达式真假。你要把业务规则写成明确条件。'),
      },
      {
        title: tr('每条路只管一种情况'),
        body: tr('先写清楚 if 负责什么，再写 else 或兜底负责什么。不要让两个分支输出同一类结果。'),
      },
      {
        title: tr('边界值最容易露馅'),
        body: tr('大于、小于、等于这几个符号决定了临界点归谁。测试经常盯的就是这些地方。'),
      },
    ]
  }
  ...
  if (/循环|for|while|array|list|数组|列表|foreach|vector|map/.test(text)) {
    return [
      {
        title: tr('循环是可控重复'),
        body: tr('循环不是让代码转圈，而是把同一个动作稳定作用到一组数据上。每轮只处理一个清楚的小任务。'),
      },
      {
        title: tr('先集合，再规则'),
        body: tr('先确认数据从哪里来、每一项是什么，再决定每轮要打印、累计、筛选还是转换。'),
      },
      {
        title: tr('退出条件要看得见'),
        body: tr('while 或无限序列最怕没有退出条件。写循环时先问自己：它什么时候停？停不下来就是事故。'),
      },
    ]
  }
  ...
  if (/函数|方法|function|method|add|return/.test(text)) {
    return [
      {
        title: tr('函数是一台小机器'),
        body: tr('参数是输入，函数体是处理规则，return 或输出是交付。边界清楚，复用才有意义。'),
      },
      {
        title: tr('命名暴露设计水平'),
        body: tr('函数名应该说明动作，参数名说明数据。读者不该靠猜测理解这段逻辑。'),
      },
      {
        title: tr('定义和调用是两件事'),
        body: tr('只写函数定义不会自动执行。必须在合适位置调用它，测试才会看到结果。'),
      },
    ]
  }
  ...
  if (/class|对象|属性|property|user|counter|constructor|构造/.test(text)) {
    return [
      {
        title: tr('对象是有记忆的模型'),
        body: tr('属性保存状态，方法改变或读取状态。把相关数据和动作放在一起，项目才不会越写越散。'),
      },
      {
        title: tr('构造阶段先把状态立住'),
        body: tr('对象创建时必须把关键字段初始化清楚。后面每个方法都默认这些状态可靠存在。'),
      },
      {
        title: tr('行为要围绕状态'),
        body: tr('别为了写 class 而写 class。方法应该让对象完成真实动作，比如更新计数、生成描述或执行业务规则。'),
      },
    ]
  }
  ...
  if (/指针|内存|raii|unique_ptr|引用|reference/.test(text)) {
    return [
      {
        title: tr('地址不是值本身'),
        body: tr('值存在内存里，地址只是位置。通过指针或引用操作时，先确认你拿到的是谁、它还活着没有。'),
      },
      {
        title: tr('所有权决定谁收尾'),
        body: tr('资源要有负责人。没有明确所有权，泄漏、重复释放和悬空引用会把问题藏到最难排查的地方。'),
      },
      {
        title: tr('先写保守代码'),
        body: tr('底层能力不是炫技场。先让生命周期清楚、边界可见，再考虑优化。'),
      },
    ]
  }
  ...
  if (/async|await|异步|fetch|promise/.test(text)) {
    return [
      {
        title: tr('等待是流程的一部分'),
        body: tr('异步代码不是乱序魔法。它只是把“现在拿不到结果”的任务标出来，等结果回来再继续。'),
      },
      {
        title: tr('await 后才是真值'),
        body: tr('没有 await 时，你手里可能只是未来结果的承诺。要明确每一步拿到的到底是 Promise 还是数据。'),
      },
      {
        title: tr('失败路径要提前想'),
        body: tr('网络、文件、接口都会失败。异步流程没有错误处理，就等于把用户体验交给运气。'),
      },
    ]
  }
  ...
  return [
    {
      title: tr('先抓住「{concept}」').replace('{concept}', tr(concept)),
      body: tr('这关的语法只是工具，真正训练的是把目标拆成输入、处理、输出，并用 {languageName} 写成能验证的代码。').replace('{languageName}', languageName),
    },
    {
      title: tr('读目标，不读感觉'),
      body: tr('目标是：{objective}。你写的每一行都应该服务这个目标，暂时无关的东西先别放进来。').replace('{objective}', tr(level.objective)),
    },
    {
      title: tr('最小版本先赢'),
      body: tr('先写出能跑的最小形态，例如从 `{coreLine}` 这种核心结构开始，再一点点补清楚。').replace('{coreLine}', coreLine),
    },
  ]
}

function reviewQuestions(languageName: string, level: Level, concept: string, tr: Tr) {
  return [
    tr('这关的「{concept}」解决了 {objective} 里的哪一部分？').replace('{concept}', tr(concept)).replace('{objective}', tr(level.objective)),
    tr('如果只保留最小代码，哪一行是 {languageName} 程序真正交付结果的关键？').replace('{languageName}', languageName),
    tr('测试失败时，你会先检查入口、数据、处理、输出里的哪一段？为什么？'),
  ]
}
...
export function getLevelMission(languageName: string, level: Level, tr: Tr = identityTr): LevelMission {
  const context = contextFor(languageName)
  const scene = LEVEL_SCENES[(level.id - 1) % LEVEL_SCENES.length]
  const intent = levelIntent(level, tr)

  return {
    kicker: `${languageName} Mission · Lv.${level.id}`,
    brief: tr('{scene}：在 {arena} 里，{intent}。')
      .replace('{scene}', tr(scene))
      .replace('{arena}', tr(context.arena))
      .replace('{intent}', intent),
    constraint: tr('限制：只用本关核心语法完成，先别堆无关框架。{tone}').replace('{tone}', tr(context.tone)),
    payoff: tr('交付：{artifact}，并让测试能明确判断它做对了。').replace('{artifact}', tr(context.artifact)),
  }
}

export function getLevelTeachingBlueprint(languageName: string, level: Level, tr: Tr = identityTr): TeachingBlueprint {
  const context = contextFor(languageName)
  const syntax = syntaxVocabulary(languageName, level)
  const coreLine = firstCodeLine(level)

  return {
    concept: tr(syntax.concept),
    realUse: tr('在 {arena} 里，这类能力用来{intent}。学它不是为了背语法，而是为了把一个小目标稳定交付出来。')
      .replace('{arena}', tr(context.arena))
      .replace('{intent}', levelIntent(level, tr)),
    mentalModel: tr(syntax.mentalModel),
    walkthrough: teachingWalkthrough(languageName, level, syntax.concept, tr),
    learnFirst: syntax.learnFirst.map((item) => tr(item)),
    practiceSteps: [
      tr('先用一句话复述目标：{objective}').replace('{objective}', tr(level.objective)),
      tr('在脑子里画出最小结构：入口/数据/处理/输出，别急着抄长代码。'),
      tr('先写最小可运行版本，核心形态可以参考：{coreLine}').replace('{coreLine}', coreLine),
      tr('运行一次，只看第一个报错或第一个失败测试。一次只修一处。'),
    ],
    pitfalls: syntax.pitfalls.map((item) => tr(item)),
    vocabulary: syntax.vocabulary.map((item) => ({ term: tr(item.term), meaning: tr(item.meaning) })),
    reviewQuestions: reviewQuestions(languageName, level, syntax.concept, tr),
    checkpoint: tr('你通关时应该能解释：为什么这段 {languageName} 代码满足目标、每个测试为什么会通过、下一关会复用哪块能力。').replace('{languageName}', languageName),
    stretchGoal: tr('加一点自己的变化，但别改坏目标：换一组变量名、换一个输入值，或者多打印一条解释性输出。'),
  }
}

export function getCourseNodeHook(languageName: string, node: CourseNode, tr: Tr = identityTr) {
  const text = `${node.title} ${node.objective} ${node.tags.join(' ')}`.toLowerCase()

  if (/game|pygame|sprite|collision|游戏/.test(text)) {
    return tr('{languageName} 在这里变成可玩的东西：输入、状态、反馈和失败条件都会立刻暴露你的逻辑是否靠谱。').replace('{languageName}', languageName)
  }
  if (/vision|opencv|image|ocr|图像|视觉|cv/.test(text)) {
    return tr('这个方向适合做“看得见结果”的项目：图片进去，结构化信息出来，错了也很直观。')
  }
  if (/ai|ml|llm|rag|model|机器学习|智能/.test(text)) {
    return tr('这条线会把代码接到模型、数据和产品逻辑上，重点不是炫 AI，而是让输出可控。')
  }
  if (/web|api|http|backend|后端/.test(text)) {
    return tr('把它想成一个真实服务的切片：请求进来，数据处理，结果返回，坏输入也不能把系统打穿。')
  }
  if (/data|sql|pandas|report|数据|报表/.test(text)) {
    return tr('这里的乐趣来自“脏数据变清楚”：最后能产出图表、报表或一个可以做决定的指标。')
  }
  if (/test|devops|ci|docker|deploy|测试|部署/.test(text)) {
    return tr('这条线不花哨，但很有职业味：让代码不靠祈祷运行，让发布不靠手抖。')
  }
  if (/security|network|socket|crypto|安全|网络/.test(text)) {
    return tr('这里更像侦查任务：读信号、找异常、收证据，再用代码把流程固定下来。')
  }
  if (/finance|business|业务|金融|量化/.test(text)) {
    return tr('把代码塞进真实业务流程：少算错、少复制粘贴、少让 Excel 表格半夜咬人。')
  }
  if (node.levelId) {
    return tr('这是 {languageName} 的基础零件。它看起来小，但后面每条专业路线都会默认你已经会了。').replace('{languageName}', languageName)
  }

  return tr('这不是摆设节点：学完后应该能做出一个小交付，而不是只会复述概念。')
}

export function getCourseProjectPreview(languageName: string, node: CourseNode, tr: Tr = identityTr) {
  const text = `${node.title} ${node.objective} ${node.tags.join(' ')}`.toLowerCase()

  if (node.levelId) return tr('最终用途：把这个基础点接进 {languageName} 的项目骨架，后面写工具、服务或界面时不会反复卡入口。').replace('{languageName}', languageName)
  if (/game|pygame|sprite|collision|游戏/.test(text)) return tr('项目预览：一个带输入、碰撞、计分和失败条件的 2D 小游戏。')
  if (/vision|opencv|image|ocr|图像|视觉|cv/.test(text)) return tr('项目预览：图片/视频检测流水线，能输出识别结果和统计报告。')
  if (/ai|ml|llm|rag|model|机器学习|智能/.test(text)) return tr('项目预览：一个能读资料、给答案、保留引用的智能助手。')
  if (/web|api|http|backend|后端/.test(text)) return tr('项目预览：带鉴权、数据库和错误处理的 API 服务。')
  if (/data|sql|pandas|report|数据|报表/.test(text)) return tr('项目预览：从原始数据到可筛选报表和关键指标的分析台。')
  if (/test|devops|ci|docker|deploy|测试|部署/.test(text)) return tr('项目预览：一套能自动测试、构建、发布的小型工程流水线。')
  if (/security|network|socket|crypto|安全|网络/.test(text)) return tr('项目预览：日志扫描、异常定位和风险报告自动化工具。')
  if (/finance|business|业务|金融|量化/.test(text)) return tr('项目预览：业务数据看板或量化回测报告，不再手搓表格。')

  return tr('项目预览：一个能被别人打开、输入、运行、得到结果的小型交付。')
}
...
export function getRuntimeModeCopy(
  languageName: string,
  ...
  if (runtime === 'python-pyodide') {
    return {
      title: tr('真实运行模式'),
      body: tr('{lang} 代码会在浏览器内执行，输出和错误来自真实解释器。').replace('{lang}', languageName),
    }
  }
  ...
  return {
    title: tr('结构化练习检查器'),
    body: tr('{lang} 当前先验证入口、类型、块结构和核心意图。它不是完整编译器，但足够帮你练语法骨架；真正工程运行会在后续沙箱/编译器接入。').replace('{lang}', languageName),
  }
}
