      ...
      <header className="sticky top-0 z-20 border-b border-cyan-300/12 bg-background/84 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2 text-xs text-white/30">
            ...
            <span>{snippet.view_count + 1} {translate('次查看', lang)}</span>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 space-y-6 px-4 py-10">
          ...
          {level && (
            <div className="flex items-center gap-2">
              ...
              <span className="rounded border border-cyan-300/22 bg-cyan-300/[0.06] px-2 py-0.5 text-xs font-medium text-cyan-100/70">
                Lv.{level.id} · {translate(level.badge, lang)}
              </span>
            </div>
          )}
          <h1 className="text-2xl font-bold">
            {snippet.title ?? `${language} ${translate('代码片段', lang)}`}
          </h1>
          <p className="text-white/40 text-sm">
            {translate('分享于', lang)} {new Date(snippet.created_at).toLocaleDateString(lang === 'en' ? 'en-US' : 'zh-CN', { year: 'numeric', month: 'long', day: 'numeric' })}
            {' · '}{translate('片段 ID:', lang)} <code className="font-mono text-cyan-300/70">{id}</code>
          </p>
        </div>
        ...
        {snippet.mentor_quote && (
          <section className="rounded-lg border border-cyan-300/18 bg-cyan-300/[0.045] p-5">
            ...
            <p className="mt-3 text-xs text-white/34">
              {snippet.codename ?? translate('匿名玩家', lang)} {translate('被导师点评后公开了这张通关卡。', lang)}
            </p>
          </section>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="text-xs text-white/30 uppercase tracking-widest">{translate('代码', lang)}</span>
            ...
            <div className="rounded-xl overflow-hidden border border-white/8 bg-[#0a0a12]">
              ...
              <pre className="p-5 font-mono text-sm text-cyan-50/78 leading-relaxed overflow-x-auto whitespace-pre">
                {displayLines.join('\n')}
                {truncated && (
                  <span className="text-white/25">{'\n'}... ({codeLines.length - 40} {translate('行已省略)', lang)}</span>
                )}
              </pre>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {(snippet.output_text || snippet.output_image) && (
              <>
                <span className="text-xs text-white/30 uppercase tracking-widest">{translate('运行输出', lang)}</span>
            ...
            <div className="rounded-lg border border-cyan-300/18 bg-cyan-300/[0.055] p-5 space-y-3 text-center">
              <p className="text-sm font-semibold text-white">{translate('想自己被导师扎一下？', lang)}</p>
              <p className="text-xs text-white/50">{translate('先免登录试玩，跑通第一课再决定要不要保存进度。', lang)}</p>
              <Link
                href="/play"
